...
from app.schemas.extraction_types import (
    CandidateBox,
    CompoundOccurrence,
    CompoundPatent,
    ExtractionTuning,
...
class ExtractionService:
    ...
    def extract_candidate_compounds(
        ...
        try:
            for page_number, page in enumerate(document, start=1):
                ...
                for compound in cls.detect_in_image(image_bgr, page_number=page_number, tuning=tuning):
                    patent.add_compound(compound)
    ...
    def detect_in_image(
        ...
        """Detect and crop candidate chemical structures in a single BGR image array.

        Shared by both the PDF page loop and the standalone-image path (PNG/JPG and
        PPTX-embedded pictures), so every input format runs the same detector.
        Returns one CompoundOccurrence per surviving candidate, each carrying the
        cropped PNG bytes and the ``page_number`` (used as page/slide index).
        """
        cv2, _, np = cls._require_extractor_dependencies()
        tuning = tuning or ExtractionTuning()
        ...
        compounds: list[CompoundOccurrence] = []
    ...
    @classmethod
    def detect_in_image_bytes(
        cls,
        image_bytes: bytes,
        location_index: int = 1,
        tuning: Optional[ExtractionTuning] = None,
    ) -> list[CompoundOccurrence]:
        """Decode raw image bytes (PNG/JPG/etc.) and run structure detection on them.

        Standalone images (and PPTX-embedded pictures) are typically already cropped
        tightly around a single structure, which would fill the frame and trip the
        detector's ``max_page_fraction`` guard (designed for small regions inside a
        large page). We add a white border so the structure occupies a sub-region,
        matching the detector's page-scale assumptions.
        """
        cv2, _, np = cls._require_extractor_dependencies()
        ...
        border = int(round(0.3 * max(width, height)))
        padded = cv2.copyMakeBorder(
            image_bgr, border, border, border, border,
            cv2.BORDER_CONSTANT, value=(255, 255, 255),
        )
        return cls.detect_in_image(padded, page_number=location_index, tuning=tuning)
